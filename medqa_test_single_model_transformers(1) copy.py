...
def load_model(model_path: str):
    """Load model - supports both GGUF (llama-cpp) and transformers."""
...
def generate_answers_gguf(
    ...
    """Generation using llama-cpp (GGUF models)."""
    texts = []
...
def evaluate_model(
    ...
    # GGUF models process one at a time, not batched
    effective_batch = 1 if is_gguf else batch_size

    for start in tqdm(range(0, len(items), effective_batch), desc="Evaluating"):
        chunk = items[start : start + effective_batch]
        prompts = [create_prompt(q, opt) for (_, q, opt, _) in chunk]

        try:
            if is_gguf:
                responses = generate_answers_gguf(
                    model=model,
                    prompts=prompts,
                    max_new_tokens=max_new_tokens,
                    temperature=temperature,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        help="HF model path or repo id, or .gguf file path",
    )
